CLASS zdmo_gen_dt266_single DEFINITION
...
**************************************************************************
  ...
  PRIVATE SECTION.
    ...
    CONSTANTS:
      co_prefix                     TYPE string           VALUE 'ZDT266_',
      co_entity_name                TYPE string           VALUE 'Carrier',
      co_session_name               TYPE string           VALUE 'DT266',
...
*      co_super_package_ext_bo       TYPE sxco_package     VALUE 'ZLOCAL'
      co_super_package_base_bo      TYPE sxco_package     VALUE 'ZDT266',
      co_super_package_ext_bo       TYPE sxco_package     VALUE 'ZDT266_EXT'
      .


    CONSTANTS location_letter TYPE c LENGTH 1 VALUE '0'.
...
CLASS zdmo_gen_dt266_single IMPLEMENTATION.
...
*    lo_specification->properties->set_super_package( 'ZDT261' )->set_software_component( co_software_component_base_bo ).
    DATA(lo_result) = lo_put_operation->execute( ).
    DATA(my_package) = xco_lib->get_package(  package_name  ).
    "fails in 2508 and 2511 in no hotfix is being applied
    "workaround - run execute method a second time
...
*    DATA(ls_package) = xco_cp_abap_repository=>package->for( co_zlocal_package )->read( ).
  ...
  METHOD generate_vh_custom_entity.
    ...
    DATA(lo_put_operation) = get_put_operation( vh_environment ).             ""ZDT261I_VH_Product_{ unique_suffix }"
    DATA(lo_interface_specification) = lo_put_operation->for-ddls->add_object( |ZDT261I_VH_Product_{ unique_suffix }|
...
*    DATA(lo_put_operation) = get_put_operation( package_environment ).
    ...
    lo_public_section->add_type( 't_products'
        ...
        io_row_type = xco_cp_abap_dictionary=>table_type( |ZDT261I_VH_Product_{ unique_suffix }| )
  ...
  ( |   ). | )
    ...
    lo_specification->implementation->add_method( |if_rap_query_provider~select|
        ...
        ( |  DATA business_data TYPE TABLE OF ZDT261I_VH_Product_{ unique_suffix }.| )
        ( |  DATA business_data_line TYPE ZDT261I_VH_Product_{ unique_suffix } .       | )
...
|    "extensibilityElementSuffix":"ZAB",\r\n| &&


|"draftTable":"ZDT266_CRR_{ unique_suffix }_D",\r\n| &&
|"cdsInterfaceView":"ZI_DT266_CARR_{ unique_suffix }",\r\n| &&

|"cdsRestrictedReuseView":"ZR_DT266_CARR_{ unique_suffix }", \r\n| &&
|"cdsProjectionView":"ZC_DT266_CARR_{ unique_suffix }",\r\n| &&

|"metadataExtensionView":"ZC_DT266_CARR_{ unique_suffix }", \r\n| &&
|"behaviorImplementationClass":"ZBP_R_DT266_CARR_{ unique_suffix }", \r\n| &&

|"sapobjectnodetype":"ZDT266_Carrier_{ unique_suffix }", \r\n| &&
|"extensioninclude":"ZSDT266_CARR_{ unique_suffix }", \r\n| &&
|"extensionincludeview":"ZE_DT266_CARR_{ unique_suffix }", \r\n| &&
|"draftqueryview":"ZR_DT266_CARR_{ unique_suffix }_D", \r\n| &&
*|"extensibilityElementSuffix":"ZAA", \r\n| &&
|"serviceDefinition":"ZUI_DT266_CARR_{ unique_suffix }_O4", \r\n| &&
...
*|       \{\r\n| &
*|           "alias": "Product",\r\n| &
*|           "name": "ZDT261I_VH_Product_{ unique_suffix }",\r\n| &
...
'        {' && |\r\n|  &&
'            "abapfieldname": "NAME",' && |\r\n|  &&
'            "dataelement": "/dmo/carrier_name",' && |\r\n|  &&
'            "isdataelement": true,' && |\r\n|  &&
'            "cdsviewfieldname": "Name"' && |\r\n|  &&
'        },' && |\r\n|  &&
...
'        },' && |\r\n|  &&
'        {' && |\r\n|  &&
'            "abapfieldname": "LOCAL_CREATED_BY",' && |\r\n|  &&
'            "dataelement": "abp_creation_user",' && |\r\n|  &&
'            "cdsviewfieldname": "CreatedBy",' && |\r\n|  &&
'            "isdataelement": true' && |\r\n|  &&
'        },' && |\r\n|  &&
'        {' && |\r\n|  &&
'            "abapfieldname": "LOCAL_CREATED_AT",' && |\r\n|  &&
...
*    ascii_hex_string = substring( val = ascii_hex_string off = strlen( ascii_hex_string ) - 3 len = 3 ).
    ...
    WHILE is_valid_package = abap_false.
      ...
      IF NOT lo_package->exists( ) AND
        ...
        s_unique_suffix = location_letter && group_id.
      ELSE.
        group_id += 1.
*        ascii_hex_string = ascii_hex.
*        ascii_hex_string = substring( val = ascii_hex_string off = strlen( ascii_hex_string ) - 3 len = 3 ).
...
*    transport = 'D23K900976'. " <-- maintain your transport request here
    ...
    "database tables
    table_name_root               = to_upper( |{ co_prefix }_CARR_{ unique_suffix }| ).
    draft_table_name_root         = to_upper( |{ co_prefix }_CRR{ unique_suffix }_D| ).
    "CDS views
    r_view_name_travel            = to_upper( |ZR_{ co_prefix }_CARR_{ unique_suffix }| ).
    c_view_name_travel            = to_upper( |ZC_{ co_prefix }_CARR_{ unique_suffix }| ).
    i_view_name_travel            = |ZI_{ co_prefix }_CARR_{ unique_suffix }|.
    i_view_basic_name_travel      = |{ co_prefix }I_{ co_entity_name }_{ unique_suffix }|.
    "behavior pools
    beh_impl_name_travel          = |ZBP_R_{ co_prefix }_CARR_{ unique_suffix }|.
    "business service
    srv_definition_name           = |ZUI_{ co_prefix }UI_CARR_{ unique_suffix }_O4|.
    srv_binding_o4_name           = |ZUI_{ co_prefix }UI_CARR_{ unique_suffix }_O4|.
...
**        xco_runtime_exception->
    ...
    data_generator_class_name     = |zDT266_cl_vh_product_{ unique_suffix }|.
