CLASS zdmo_cl_rap_bo_delete DEFINITION
...
*
    TYPES: BEGIN OF t_deletion_sequence_line,
             sequence_number TYPE int4,
             object_TYPE     TYPE if_xco_gen_o_finding=>tv_object_type,
           END OF t_deletion_sequence_line.

    TYPES: t_deletion_sequence TYPE STANDARD TABLE OF t_deletion_sequence_line WITH EMPTY KEY.

    DATA deletion_sequence TYPE t_deletion_sequence.

    DATA package TYPE sxco_package.

    DATA my_out  TYPE REF TO if_oo_adt_classrun_out.
...
*      RAISING   cx_bali_runtime
    ...
    METHODS delete_generated_objects_2
      IMPORTING
        i_repository_objects TYPE zdmo_cl_rap_generator=>t_generated_repository_objects.
...
*    DATA log_entry TYPE STRUCTURE FOR CREATE ZDMO_R_RAPG_ProjectTP\\log   .
  ...
  METHOD add_text_to_app_log_or_console.
    ...
    IF my_out IS NOT INITIAL.
      LOOP AT log_entries INTO DATA(log_entry_line).
        my_out->write( |severity { log_entry_line-severity } { log_entry_line-text }| ).
      ENDLOOP.
    ENDIF.
...
*      delete_structures( repository_objects_in_transprt ).
    ...
    "fill table deletion_order
    deletion_sequence = VALUE t_deletion_sequence(
  ( sequence_number = 1 object_TYPE = 'SRVB' )
  ...
  METHOD delete_behavior_definitions.
    ...
    "severity S  Type: APIS Name: ZDT261R_SHOPTP_U01

*    SELECT single * from

    LOOP AT del_repository_objects INTO DATA(del_repository_object) WHERE is_released = abap_true.
...
*        CATCH cx_bali_runtime.( xco_delete_exception->findings ).
  ...
  METHOD delete_draft_tables.
    ...
    DATA del_repository_objects TYPE zdmo_cl_rap_generator=>t_generated_repository_objects  .
    LOOP AT i_repository_objects INTO DATA(repository_object) WHERE object_type = object_type.
      DATA(structure) = xco_lib->get_structure( CONV #( repository_object-object_name ) ).

      IF structure->exists(  ).
        add_text_to_app_log_or_console( |{ object_type } { object_name } is structure and will not be deleted in this step.| ).
      ELSE.
        APPEND repository_object TO del_repository_objects.
        object_name = to_upper( repository_object-object_name ).
...
*        CATCH cx_bali_runtime.( xco_delete_exception->findings ).
  ...
  METHOD delete_structures.
    ...
    LOOP AT i_repository_objects INTO DATA(repository_object) WHERE object_type = object_type.
      ...
      ELSE.
        add_text_to_app_log_or_console( |{ object_type } { object_name } is a table and will not be deleted in this step.| ).
      ENDIF.
...
*                                              INTO TABLE @DATA(objects_in_package) .
    ...
    LOOP AT r_repository_objects INTO DATA(r_repository_object) WHERE object_type = 'APIS'.

      DATA(my_string) = r_repository_object-object_name .
      SPLIT my_string AT space INTO TABLE DATA(string_tab).

      LOOP AT string_tab INTO DATA(line).
        CONDENSE line.
        IF line = ''.
          DELETE string_tab INDEX sy-tabix.
        ENDIF.
      ENDLOOP.

      DATA(test_name) = string_tab[ 1 ].
      DATA(test_type) = string_tab[ 2 ].

      LOOP AT r_repository_objects ASSIGNING FIELD-SYMBOL(<w_api>) WHERE object_name = test_name
                                                                     AND object_type = test_type.
        <w_api>-is_released = abap_true.
      ENDLOOP.
...
*        ENDIF.
        ...
        add_text_to_app_log_or_console( |checking package { package }| ).
...
*        IF line_exists( objects_to_be_deleted[ object_type = zdmo_cl_rap_node=>root_node_object_types-service_binding ]  ).
          IF object_to_be_deleted-object_type = zdmo_cl_rap_node=>root_node_object_types-service_binding.
            ...
            IF xco_lib->service_binding_is_published( CONV sxco_srvb_object_name(  service_binding_to_be_deleted-object_name ) ).
              ...
              IF demo_mode = abap_false.
                ...
                COMMIT WORK.
                IF xco_lib->service_binding_is_published( CONV sxco_srvb_object_name(  service_binding_to_be_deleted-object_name ) ) = abap_true .
                  "must not happen
                  ASSERT 1 = 2.
                ENDIF.
              ENDIF.
        ...
        delete_release_states( objects_to_be_deleted ).
        COMMIT WORK.
...
*      CATCH cx_bali_runtime INTO DATA(application_log_exception).
  ...
  METHOD delete_release_states.

    CHECK i_repository_objects IS NOT INITIAL.

    LOOP AT i_repository_objects INTO DATA(del_repository_object) WHERE is_released = abap_true.

      delete_release_state( EXPORTING object_type                = del_repository_object-object_type
                                      object_name                = del_repository_object-object_name
                                      request                    = del_repository_object-transport_request
                            RECEIVING
